...
def read_model_coefficients(
    ...
    if coefficients.isnull().any().any():
        logger.warning(
            f"null coefficients in {file_path}\n{coefficients[coefficients.isnull().any(axis=1)]}"
        )
...
def dump_mapped_coefficients(state: workflow.State, model_settings):
    ...
    for c in template_df.columns:
        template_df[c] = template_df[c].map(coefficients_df.iloc[:, 0])
...
def get_segment_coefficients(
    ...
    if legacy:
        ...
        coefficients_col = (
            template_df[segment_name].map(coefficients_df.iloc[:, 0]).astype(float)
        )
